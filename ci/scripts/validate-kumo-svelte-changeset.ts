...
/**
 * Validates that a changeset exists for the kumo-svelte package when source
 * files are modified in a pull request.
 */

const KUMO_PACKAGE_NAME = "@axelrock/kumo-svelte";
...
function main() {
  console.log(`Validating changeset for package: ${KUMO_PACKAGE_NAME}`);
  ...
  // design, their diff modifies package files (version bump + changelog)
  ...
  // Check if package source files have been modified
  const hasKumoChanges = checkForKumoChanges();
  if (!hasKumoChanges) {
    console.log(
      "No package source changes detected, skipping changeset validation",
    );
    return;
  }

  console.log("Package source changes detected");
  ...
  if (newKumoChangesets.length === 0) {
    ...
    if (newChangesets.length === 0) {
      console.error(
        "\x1b[31;1mERROR: Package source changes detected but no NEW changeset files found\x1b[0m",
      );
    } else {
      console.error(
        `\x1b[31;1mERROR: Found NEW changeset files, but none target ${KUMO_PACKAGE_NAME}\x1b[0m`,
      );
      console.error("");
    ...
    console.error(
      `   2. Select "\x1b[36m${KUMO_PACKAGE_NAME}\x1b[0m" when prompted`,
    );
    console.error(
    ...
    console.error("");
    console.error(
      "This ensures proper versioning and changelog generation for the package.",
    );
    console.error("");
    if (process.env.CI) {
      console.error(
  ...
  console.log(
    `Found ${newKumoChangesets.length} NEW changeset(s) for ${KUMO_PACKAGE_NAME}:`,
  );
...
function checkForKumoChanges(): boolean {
  const results = KUMO_PATHS.map((path) => hasChangesInPath(path));

  if (results.some((result) => result === null)) {
    console.warn(
      "Warning: Could not determine if package changes exist, assuming they do",
    );
    return true;
  }
...
function getNewlyAddedChangesets(): ChangesetFile[] {
  ...
  const newFiles = getNewlyAddedFiles(CHANGESET_DIR);
  ...
  for (const { status, path: filePath } of newFiles) {
    ...
    try {
      const fullFilePath = join(".", filePath);
