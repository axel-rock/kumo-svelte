...
const WORKER_URL =
  ...
  "https://kumo-svelte-screenshot-worker.workers.dev";
...
async function postPRComment(body: string): Promise<void> {
  ...
  const repo = process.env.GITHUB_REPOSITORY ?? "maxfarrell/kumo-svelte";
...
async function main(): Promise<void> {
  ...
  const beforeUrl = process.env.BEFORE_URL ?? "https://kumo-svelte.com";
  const afterUrl =
