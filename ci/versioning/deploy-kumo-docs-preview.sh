...
echo "  Using wrangler version: $(pnpm --filter @axelrock/kumo-svelte exec wrangler --version)"

# Upload version - capture output regardless of exit code
COMMIT_SHORT="${GITHUB_SHA:0:7}"
COMMIT_SHORT="${COMMIT_SHORT:-local}"
VERSION_OUTPUT=$(pnpm --filter @axelrock/kumo-svelte exec wrangler versions upload --message "Preview for ${COMMIT_SHORT}" 2>&1) || true
