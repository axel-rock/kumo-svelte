<script lang="ts">
  ...
  let activePath = $derived(normalizePathname(currentPath));
  ...
  function saveScroll(event: Event) {
    ...
    sessionStorage.setItem('kumo-sidebar-scroll', target.scrollTop.toString());
  }
...
{#snippet navLink(item: NavItem, nested = false)}
  <li>
    <a
      ...
      class={cn(LI_STYLE, nested && 'pl-4', (nested ? activePath === normalizePathname(item.href) : isActivePath(activePath, item.href)) && LI_ACTIVE_STYLE)}
    >
      {item.label}
    </a>
  </li>
...
{#snippet navSection(title: string, items: NavItem[], open: boolean, setOpen: (value: boolean) => void, maxHeight = 'max-h-[500px]')}
  <div class="mb-4">
    <button
      type="button"
    ...
    >
      <span>{title}</span>
      <ChevronDown size={12} class={cn('text-kumo-subtle transition-transform duration-200', !open && '-rotate-90')} />
    </button>
    <ul class={cn('mt-1 flex flex-col gap-px overflow-y-hidden overflow-x-visible transition-all duration-300 ease-in-out', open ? `${maxHeight} opacity-100` : 'max-h-0 opacity-0')}>
...
{#snippet navContent()}
  ...
  {@render navSection('Components', componentItems, componentsOpen, (value) => (componentsOpen = value), 'max-h-[2000px]')}
  {@render navSection('Charts', chartItems, chartsOpen, (value) => (chartsOpen = value))}
  {@render navSection('Blocks', blockItems, blocksOpen, (value) => (blocksOpen = value))}
{/snippet}
...
>
  <div class="flex h-12 flex-none items-center justify-between border-b border-kumo-hairline px-3">
    <h1 class="text-base font-medium">Kumo</h1>
    <Button variant="ghost" shape="square" aria-label="Close menu" onclick={() => (mobileMenuOpen = false)}>
      <X size={20} />
    </Button>
  </div>
  <div bind:this={mobileScrollRef} data-sidebar-scroll="mobile" class="min-h-0 grow overflow-y-auto overscroll-contain px-3 py-4 text-sm text-kumo-subtle" style:scroll-behavior="auto" onscroll={saveScroll}>
    {@render navContent()}
  </div>
</aside>
...
<div class="pointer-events-none fixed top-0 left-12 z-50 hidden h-12 items-center px-3 font-medium select-none md:flex">
...
<aside
  ...
  class={cn(
    ...
    sidebarOpen ? 'translate-x-0 border-r border-kumo-hairline' : '-translate-x-full'
  )}
>
  <div class="h-12 flex-none border-b border-kumo-hairline"></div>
  <div bind:this={desktopScrollRef} data-sidebar-scroll="desktop" class="min-h-0 grow overflow-y-auto overscroll-contain px-3 py-4 text-sm text-kumo-subtle" onscroll={saveScroll}>
    {@render navContent()}
  </div>
</aside>
